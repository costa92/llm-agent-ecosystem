...
| Independently shippable | Yes — culminating in v2.0.0 tag at M8c. **Sub-milestone split (v2 of umbrella spec, after 2-round review found 20 issues in v1):** M8a-prep (migration framework v2 + relay hardening + event-type allowlist) → M8a (substrate: Working tier expand-first rollout + `Promoter`/`Deduper` interfaces + dedupe primitive + LastAccessAt) → M8b (Consolidation Worker, new sibling) ∥ M8c (Phase E storage refactor: scoredStore package-cluster lift + benchmark + concurrency refactor + VectorBackend + snapshot v2) → M8d (typed RecallObserver + reason-enum freeze + remaining 7 counters). Each sub-milestone gets its own spec + two-round review before its impl plan. |
| Umbrella spec | `docs/superpowers/specs/2026-05-27-m8-umbrella-design.md` |
| Exit criteria | 1. `scoredStore` no longer takes a single `sync.Mutex`; either RWMutex+immutable-iter-view, CoW, or shard-by-kind. Benchmark shows ≥2× concurrent-read throughput at 10k items. 2. Episodic/Semantic accept pluggable `VectorBackend`; pgvector backend implemented and wired to the existing Postgres deployment. 3. Scope becomes a primary column on `memory_record`; in-memory store gains a parallel partition key; "filter at result side" code paths are deleted. 4. `MemoryItem` v2 lifts `Source` / `Category` / `Pinned` / `Disabled` / `Scope` out of `Metadata` to typed fields. 5. Snapshot version bumped to `2`; migration tool reads v1 and writes v2. 6. **Working tier introduced** via **expand-first** rollout (umbrella §4.1, v2): Phase 1 adds `('working','episodic','semantic')` CHECK as `NOT VALID` in M8a-prep; Phase 2 `VALIDATE`s off-peak; Phase 3 (M8a) opts new writes into `kind='working'` via SDK helper, existing rows untouched; Phase 4 (M8b) worker promotes by setting `kind='episodic'` AND `consolidated_from_event_id` — provenance for "was promoted" is the latter column, NOT `kind`; Phase 5 (M8d-tail) drops the old constraint. 7. **`Promoter` interface (new, additive)** — separate SDK interface, not a `RecordStore` method extension (umbrella §4.2 v2 — avoids breaking 3 existing test mocks + any v1.0.0 downstream); idempotency via `(tenant_id, idempotency_key)` matching existing `memory_idempotency` table; emits `memory_promoted` event in one Postgres transaction. 8. **`Deduper` interface (new, additive)** — separate SDK interface with explicit loser-resolution semantics (umbrella §4.3 v2): loser is marked `deleted=TRUE`, emits `memory_dedupe_collapsed` event, vector projector cleans up loser chunks; pending outbox rows for loser no-op against the deleted record. 9. **Typed `RecallObserver`** — carries `memory_id`, `request_id`, `stage`, `reason`, `was_promoted`; gateway recall path emits via this seam. 10. **`LastAccessAt` / `HitCount` write path** — recall path persists access marks (may batch). 11. **Consolidation Worker** — new `llm-agent-memory-worker` sibling module; consumes outbox via the SDK `MessagePublisher` seam; promotes Working→Episodic with the §5.3 default rules, version-fence per §8.4, two-layer dedupe; idempotent under at-least-once + relay redelivery. 12. **Relay delivery hardening** — per-row ack or durable `processing` state with lease/heartbeat (codex C-1 mitigation). 13. **Reason enum frozen** at the SDK boundary per §13.4; freeform reasons rejected. 14. **Remaining 12 counters land** — 5 Promote, 4 Working-lifecycle (`working_expired/promoted/dropped_before_use_total`, `memory_stale_hit_total`), 1 reason-bucketed `recall_dropped_total`, plus `recall_helpful_total` / `recall_unhelpful_total` paired with a feedback-ingest endpoint on the gateway. 15. Vector-similarity dedupe (P2) gated behind an opt-in policy flag — non-default. 16. Decommission plan for `llm-agent/memory/` declared; final patch release of core memory cuts. |
| Complexity | XL (split into M8a/b/c/d per umbrella spec) |
| Detailed plan file | per sub-milestone — to be written after each sub-milestone spec passes review. Umbrella spec is the entry point. |
