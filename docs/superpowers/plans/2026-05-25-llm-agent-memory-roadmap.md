...
| M5–M7 | Untouched. | Owns Postgres schema, HTTP Gateway, workers, decision-trace. |
| M8 (v2 breaking) | **Deprecation announced.** Consumers should migrate to the v1+ subproject surfaces. | Owns scope-as-first-class-key + storage refactor + schema normalization. |
...
| Exit criteria | 1. `ManagerOptions` fields typed as interfaces: `Memory`, `Lister`, `Exporter`, `Importer`, optional `LifecycleMemory`. 2. `WithSanitizer`-wrapped memory installs into `Manager` without a cast. 3. `RecallEngine` facade exposes `Recall(ctx, query, opts) -> UnifiedRecall`; tier-awareness becomes internal. 4. Migration guide doc (`docs/memory-v1-migration.zh-CN.md`) listing every break. 5. Compatibility shim package `memory/compat/` provides old `ManagerOptions` constructor for one release window. 6. All M1–M3 tests adapted; no test deletion without justification. |
| Complexity | L |
...
| Attribute | Value |
|---|---|
| Goal | Produces the Postgres schema, migrations, and a Go data-access layer covering the 4 mandatory tables (+1 optional). Includes idempotent CRUD on `memory_record` with `expected_version` OCC and the Outbox relay loop. |
...
| Independently shippable | Yes — a usable `memory/store/pg` package and a `cmd/outbox-relay` binary. |
| Exit criteria | 1. SQL migrations create all 4 mandatory tables with §11 minimum indexes; `memory_recall_invalidation` deferred. 2. Go DAL exposes `WriteRecord` / `PatchRecord` / `DeleteRecord` / `PinRecord` / `DisableRecord` — each one a single transaction covering `memory_record` + `memory_event` + `outbox_event` + (where applicable) `memory_idempotency`. 3. OCC: every modify call requires `expected_version` and returns a typed `ErrVersionConflict` on `where version = ? returning 0`. 4. Outbox relay polls `status='pending'`, publishes to a pluggable `Publisher` interface (initial impl: in-memory + log; Kafka/NATS deferred to M7), updates `status='sent'`. 5. Idempotency: same `(tenant_id, idempotency_key, request_hash)` returns prior result; different hash returns `409 idempotency_conflict`. 6. Invariants 1–7 from §12 each covered by a test. 7. Migration tool runnable via `cmd/memory-migrate`. |
| Complexity | L |
| Detailed plan file | `docs/superpowers/plans/2026-08-XX-m5-postgres-outbox.md` |

### M6 — Memory Gateway HTTP API (First-Batch 7 Endpoints)

| Attribute | Value |
|---|---|
| Goal | Produces a runnable `cmd/memory-gateway` HTTP server exposing the §11 first-batch 7 endpoints, backed by M5 DAL and an in-memory vector index stub. |
...
| Independently shippable | Yes — Gateway binary + Go client SDK. |
...
### M7 — Consolidation Worker + 4-Class Validation Metrics + Decision Trace Persistence

| Attribute | Value |
|---|---|
| Goal | Produces the async Consolidation Worker that idempotently promotes Working→Episodic, plus the v1 4-class validation metrics (recall / promote / lifecycle / cost) and persisted decision traces. |
| Phase mapping | `docs/memory-roadmap.zh-CN.md` §11.1 P0 (validation metrics + decision trace); §11.2 P1 (Consolidation Worker + dedupe layers); `multi-service-memory-architecture.zh-CN.md` §9.12 (v1 min), §13.4 (decision trace minimum). |
| P0/P1/P2 | Validation metrics: P0. Consolidation worker: P1. Decision-trace minimum: P0. |
| Doc refs | `memory-roadmap.zh-CN.md` §11.1 (decision-trace reasons + 4-class metrics), §11.2 (workers + 2-layer dedupe); `multi-service-memory-architecture.zh-CN.md` §5.3 (promote conditions + thresholds), §5.4 (failure state machine), §9.12 (validation methods A/B/C/D), §13.4 (trace schema, reason enums, reason→metric mapping, JSON examples). |
| Independently shippable | Yes — `cmd/consolidation-worker` binary. |
| Exit criteria | 1. Worker consumes outbox events; idempotent (same event replay leaves state unchanged). 2. Two-layer dedupe: (a) by `memory_id`; (b) by `tenant_id + user_id + category + normalized_content_hash`. Vector-similarity dedupe explicitly deferred to M8+. 3. Promote default rules implemented: `user_saved` direct; `agent_inferred` requires `importance ≥ 0.7` OR ≥3 occurrences in 24h OR ≥2 distinct session refs. 4. Version-fence rule from §8.4: worker compares `event.version == current.version` before applying. 5. Recall metrics (A): `recall_returned_total`, `recall_selected_total`, `recall_dropped_total`, `recall_helpful_total`, `recall_unhelpful_total`. 6. Promote metrics (B): `promote_attempt_total`, `promote_accepted_total`, `promote_rejected_total`, `promoted_recalled_total`, `promoted_selected_total`. 7. Lifecycle metrics (C): `working_expired_total`, `working_promoted_total`, `working_dropped_before_use_total`, `episodic_disabled_total`, `episodic_deleted_total`, `memory_stale_hit_total`. 8. Cost metrics (D): `embedding_request_total`, `embedding_applied_total`, `embedding_tokens_total`, `embedding_cost_total`, `memory_storage_bytes_total`, `vector_storage_bytes_total`. 9. Decision trace persisted as structured logs/events with the full reason enum (§13.4); reason→counter mapping table emitted. 10. Cross-tenant isolation test: a forged `tenant_id` cannot read/write/promote across boundary. |
| Complexity | L |
| Detailed plan file | `docs/superpowers/plans/2026-10-XX-m7-workers-and-validation.md` |

### M8 — Phase E v2 Breaking: Scope-as-First-Class + Storage Refactor + Schema Normalization

| Attribute | Value |
|---|---|
| Goal | Produces a v2.0.0 release: scope is part of the primary storage key, `scoredStore` is replaced by sharded/RW-mutex or CoW reads, Episodic/Semantic can swap to external vector backends (pgvector first), and stable fields are promoted out of `Metadata`. |
| Phase mapping | Phase E (`docs/memory-roadmap.zh-CN.md` §6.1, items E-1/E-2/E-3/E-4). |
| P0/P1/P2 | Mostly P0+P2 scope upgrade; vector-similarity dedupe and learned features remain P2 / out-of-scope. |
| Doc refs | `memory-roadmap.zh-CN.md` §6.1 (all four E items); `multi-service-memory-architecture.zh-CN.md` §8.1 (Postgres recommended fields), §8.2.1 (vector-index reliability), §11.3 (P2 enhancements). |
| Independently shippable | Yes — v2.0.0 tag with explicit migration window. |
| Exit criteria | 1. `scoredStore` no longer takes a single `sync.Mutex`; either RWMutex+immutable-iter-view, CoW, or shard-by-kind. Benchmark shows ≥2× concurrent-read throughput at 10k items. 2. Episodic/Semantic accept pluggable `VectorBackend`; pgvector backend implemented and wired to the existing Postgres deployment. 3. Scope becomes a primary column (`memory_record` already has it from M5; the in-memory store gains a parallel partition key). "Filter at result side" code paths are deleted. 4. `MemoryItem` v2 lifts `Source` / `Category` / `Pinned` / `Disabled` / `Scope` out of `Metadata` to typed fields; `Metadata` is reserved for caller extensions. 5. Snapshot version bumped to `2`; migration tool reads v1 and writes v2. 6. Vector-similarity dedupe (P2) gated behind an opt-in policy flag — non-default. 7. Decommission plan for `llm-agent/memory/` declared; final patch release of core memory cuts. |
| Complexity | L |
...
| **Observability / decision trace** | Observer hooks land in M2 (no third-party SDK). M6 wires structured-log decision traces. M7 persists them and exposes the v1 4-class metrics. The reason enum in `multi-service-memory-architecture.zh-CN.md` §13.4 is the canonical set; freeform reasons are forbidden in v1. `memory_id` stays out of metric labels (high cardinality) — only `tenant_id` (or bucket) is allowed. |
...
| **Backwards compat with `llm-agent/memory/` consumers** | M1–M3 are pure additive — old SDK still works. M4 introduces v1 breaks, mitigated by a `memory/compat/` shim and a published migration guide. M8 cuts the v2 break with at least one minor-release deprecation window. Snapshot format gets a version bump at M8 only (v1→v2). Symptomatically: anyone on `llm-agent` v0.5+ can adopt `llm-agent-memory` up to M3 with zero code changes beyond `import` swap. |
...
| M7 | (1) Default promote thresholds (importance ≥ 0.7, ≥3 in 24h, ≥2 sessions) are unvalidated — may need per-tenant tuning. (2) 4-class metric cardinality could explode if `tenant_id` is appended carelessly. |
...
- **M7**: Which queue runtime — NATS, Kafka, or polling-only? Where do decision traces persist — same Postgres, separate sink, or just structured logs?
- **M8**: Vector backend choice — pgvector (already in rag), Qdrant, or Milvus? Migration path for v0/v1 unscoped data — back-fill with `tenant_id=legacy`?
...
- [x] Decision trace minimum (`reason` enums) on roadmap: M6 (emit) + M7 (persist + reason→counter map).
- [x] v1 4-class validation metrics (recall / promote / lifecycle / cost) on roadmap: M7 exit criteria 5–8.
