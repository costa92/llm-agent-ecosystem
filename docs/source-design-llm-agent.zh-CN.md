...
1. **rag/ 空目录 + go.mod 死依赖**：`require github.com/costa92/llm-agent-rag v1.0.1` 没被任何 `.go` 文件 import；`grep -rn 'llm-agent-rag' --include='*.go'` 只在 agentstest/doc.go 注释里出现。从严格 `stdlib-only` 角度，这违反了 CLAUDE.md 第 1 条"Core repo 必须 stdlib-only" —— umbrella README 第 76 行虽然给了豁免（"唯一允许的反向边"），但 facade 实际 *未实现*，所以这条依赖暂时纯粹是 phantom。**建议**：要么落地 facade（写 `rag/facade.go` 转发 `rag.System` / `rag.Retriever` 给 `llm-agent-rag`），要么暂时移除该依赖。
2. **`comm/a2a` 后台任务 ctx 泄露**：`a2a/server.go:128-129` 用 `context.Background()` 起 worker goroutine，注释自己写了 FIXME "future improvement: store cancel in taskStore and expose DELETE /tasks/{id}"。**长跑或大量任务下 goroutine 不可中断**。
...
5. **`runStreamFromBlocking` 在 ctx-cancel 时不发送 Done event**：`agent.go:107-122` `select { case ch<-StepEvent{...}: case <-ctx.Done(): }` —— 若在写 Done 前 ctx cancel，channel 关闭但 *consumer 收不到 final event*。consumer 在 `for ev := range ch` 里看到 channel 关闭 *没有* 区分"正常完成"与"中途 cancel"。**建议**：在 ctx 取消路径上 sender 仍尝试推一个 `StepEvent{Done:true, Err: ctx.Err()}` 进 channel（buffered 16，几乎一定能放下）。
...
> **结语**。`llm-agent` 在"stdlib-only + 小接口 + 装饰器组合"这条主线上贯彻得相当彻底；K1–K3、CC-1/CC-2、KC-1 都落到了代码层面，且大部分文件级注释都解释了 *为什么这么写*（而不只是 *怎么写*）。当前主要的缺口是：(a) RAG facade 名实不副；(b) FunctionCallAgent 单轮限制；(c) 几处 ctx propagation 与 cleanup 路径（comm/a2a 后台 goroutine、stream cancel done event）尚有 FIXME。把这三类问题集中清理掉 + 在 v1 前做完三个 *值得 breaking* 的命名调整（context 包名、Termination 归属、As*Tool 统一），核心就具备进入 *full stability commitment* 的条件。
